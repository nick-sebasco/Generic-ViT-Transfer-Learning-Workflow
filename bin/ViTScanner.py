...
def vitScan(
    ...
    use_full_ref_roi: bool
        If this is true we can disregard: normalization_ref_size, normalization_ref_strategy
    max_ref_side: int
        Even if we want to use full roi we add a limit so we don't end up with memory issues.
    save_raw_target_png
        In addition to the reference and the normalized targets you can save the raw targets as pngs.
    ...
    norm_ctx = NormContext.build(
        ...
        use_full_ref_roi=use_full_ref_roi,
        max_ref_side=max_ref_side,
    )
